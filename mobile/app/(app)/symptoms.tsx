      ...
      {resp && (
        <>
          {resp.summary && (
            <Card>
              <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}>
                ...
                {resp.urgency && (
                  <View style={{ backgroundColor: urgencyBg, paddingHorizontal: 10, paddingVertical: 4, borderRadius: radii.pill }}>
                    <Text style={{ color: "#fff", fontSize: 11, fontWeight: "700", textTransform: "uppercase" }}>⚠ {resp.urgency}</Text>
                  </View>
                )}
              </View>
            </Card>
          ...
          {resp.causes && resp.causes.length > 0 && (
            <Card>
              <Text style={{ color: colors.text, fontWeight: "700", marginBottom: 8 }}>❤️ {t("symptoms.causes")}</Text>
              {resp.causes.map((c, i) => (
          ...
          {resp.suggestions && resp.suggestions.length > 0 && (
            <Card>
              <Text style={{ color: colors.text, fontWeight: "700", marginBottom: 8 }}>{t("symptoms.suggestions")}</Text>
          ...
          {resp.ayurveda && resp.ayurveda.length > 0 && (
            <Card style={{ borderColor: colors.success + "55", backgroundColor: colors.success + "0d" }}>
              <Text style={{ color: colors.success, fontWeight: "700", marginBottom: 8 }}>🌿 {t("symptoms.ayurveda")}</Text>
          ...
          <View style={{ backgroundColor: colors.warn + "1a", borderColor: colors.warn + "55", borderWidth: 1, borderRadius: radii.md, padding: 12 }}>
            <Text style={{ color: colors.text, fontSize: 12 }}>⚠️ {t("symptoms.disclaimer")}</Text>
          </View>
        </>
      )}
    </ScrollView>
